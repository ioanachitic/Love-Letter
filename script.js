...
function crearePachet(){//de refacut
	...
	//pachet = [8,6,2,2,8,1,2,1,1,2,7,5,7,8];
	return pachet;
}
...
function PrintJ(){
	if(CarteManaC==8)
		...
		setTimeout(function(){
							...
							t1.appendChild(x1);
							x1.setAttribute("id", "a_m_img");
							setTimeout(function(){astepareTragereCarteCalculator();},800);							
							},600);
		/*t1=document.getElementById("a_m");	
		x1 =spate();
		CarteManaC=pachet[indexPachet];
...
function actiuneSingurC(CarteJucata){
	...
	if(CarteJucata==5)  PrintSingurC();
	else if(CarteJucta==8) PrintesaC();
		 else {
			 var t=document.getElementById("a_j");
			if(a2aRunda==1){
				setTimeout(function(){t.removeChild(document.getElementById("a_j_img"));},200);			
...
function actiuneSingurJ(CarteJucata){
	...
	if(CarteJucata==5)  PrintC();
	else if(CarteJucata==8) {PrintesaJ();}
		 else {
